...
export const metadata = { title: "Dashboard — VetClinic" };
      ...
      <div className="flex-1 p-4 md:p-6">
        <DashboardVet
          ...
          citasHoy={(citasHoy ?? []).map((c) => {
            const p = c.patients as { id: string; nombre: string; apellidos: string } | null;
            const t = c.treatments as { nombre: string } | null;
            return {
              id: c.id,
              fecha_hora: c.inicio,
              motivo: t?.nombre ?? c.notas ?? null,
              mascotas: p ? { nombre: `${p.nombre} ${p.apellidos}`.trim(), especie: "" } : null,
            };
          })}
          vacunasProximas={(vacunasProximas ?? []).filter((v) => v.fecha_proxima !== null).map((v) => ({
            id: v.id,
            vacuna: v.vacuna,
            fecha_proxima: v.fecha_proxima as string,
            mascotas: Array.isArray(v.mascotas) ? v.mascotas[0] : (v.mascotas as { id: string; nombre: string } | null),
          }))}
          clinicName={org?.nombre ?? "Mi Clínica Veterinaria"}
        />
      </div>
    </div>
  );
